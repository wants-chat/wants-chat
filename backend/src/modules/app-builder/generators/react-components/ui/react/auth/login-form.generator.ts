...
export const generateLoginForm = (
  ...
  variant: 'loginWithHero' | 'loginWithFeatures' | 'loginWithImage' | 'loginCentered' = 'loginCentered'
) => {
...
const LoginFormComponent: React.FC<LoginFormProps> = ({
  ...
  // Mutation for login submission
  const loginMutation = useMutation({
    mutationFn: async (data: { email: string; password: string; rememberMe: boolean }) => {
      const response = await api.post<any>('${apiRoute}/login', data);
      ...
      <div className={cn('w-full lg:w-1/2 flex items-center justify-center p-8', styles.background)}>
        <div className="w-full max-w-md">
          ...
          <div className="space-y-4 mb-6">
            <button
              className={cn(styles.card, styles.cardHover, 'w-full h-12 flex items-center justify-center rounded-lg transition-all')}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="remember" className={\`text-sm cursor-pointer \${styles.text}\`}>
                  {rememberMeText}
                </Label>
              </div>
              <button
                className={\`text-sm hover:underline \${styles.accent}\`}
...
const LoginFormComponent: React.FC<LoginFormProps> = ({
  ...
  const title = ${getField('title')};
  const logoText = ${getField('logoText')};
  const googleButtonText = ${getField('googleLoginText')};
  const githubButtonText = ${getField('githubLoginText')};
  const emailLabel = ${getField('emailLabel')};
  const emailPlaceholder = ${getField('emailPlaceholder')};
  const passwordLabel = ${getField('passwordLabel')};
  const passwordPlaceholder = ${getField('passwordPlaceholder')};
  ...
  const signInButtonText = ${getField('signInButtonText')};
  const noAccountText = ${getField('noAccountText')};
  const signUpText = ${getField('signUpHere')};
  ...
  return (
    <div className={cn("min-h-screen flex", styles.background, className)}>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className={cn(styles.card, 'w-full max-w-md rounded-xl p-8')}>
          <h1 className={\`text-3xl font-bold mb-8 \${styles.title}\`}>{title}</h1>
...
const LoginFormComponent: React.FC<LoginFormProps> = ({
  ...
  const signUpLinkText = ${getField('signUpLinkText')};
  const googleButtonText = ${getField('googleButtonText')};
  const appleButtonText = ${getField('appleButtonText')};
  const emailLabel = ${getField('emailLabel')};
  const emailPlaceholder = ${getField('emailCompanyPlaceholder')};
  const passwordLabel = ${getField('passwordLabel')};
  const passwordPlaceholder = ${getField('passwordPlaceholder')};
  const rememberMeText = ${getField('rememberMeText')};
  const forgotPasswordText = ${getField('forgotPasswordText')};
...
const LoginFormComponent: React.FC<LoginFormProps> = ({
  ...
  const title = ${getField('signInTitle')};
  const logoText = ${getField('fluxezLogo')};
  const emailLabel = ${getField('yourEmailLabel')};
  const emailPlaceholder = ${getField('emailCompanyPlaceholder')};
  const passwordLabel = ${getField('passwordLabel')};
  const passwordPlaceholder = ${getField('passwordPlaceholder')};
  const rememberMeText = ${getField('rememberMeText')};
  const forgotPasswordText = ${getField('forgotPasswordText')};
  const signInButtonText = ${getField('signInButton')};
  const noAccountText = ${getField('noAccountCentered')};
  const signUpText = ${getField('signUpText')};
        ...
        <div className={cn(styles.card, 'rounded-xl shadow-lg p-8 lg:p-10')}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                className={\`text-sm hover:underline font-medium \${styles.accent}\`}
                onClick={handleForgotPassword}
              >
                {forgotPasswordText}
              </button>
...
export default LoginFormComponent;
  ...
  return variants[variant] || variants.loginCentered;
};
