...
/**
 * Generate Create Track Form for React Native
 *
 * This is a specialized form generator for music track creation with:
 * - File upload for track (audio)
 * - File upload for cover image
 * - Album selector dropdown
 * - Genre and metadata fields
 * - Public/private toggle
 */
export function generateCreateTrackForm(
  component: Component,
  entityName: string
...
export default function CreateTrackForm({ onSuccess, options: propOptions }: CreateTrackFormProps) {
  ...
  const [trackFile, setTrackFile] = useState<UploadedFile | null>(null);
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      try {
        ...
        const response = await fetch(\`\${apiUrl}/tracks/options\`);
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => apiClient.post('/tracks', data),
    onSuccess: () => {
  ...
  const handleTrackUpload = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setTrackFile(file);
        setFormData((prev) => ({ ...prev, track_url: file.uri }));
      }
    } catch (err) {
  ...
  const handleRemoveTrack = () => {
    setTrackFile(null);
    setFormData((prev) => ({ ...prev, track_url: '' }));
  };
  ...
  const handleSubmit = () => {
    ...
    if (!formData.track_url) {
      Alert.alert('Validation Error', 'Please upload a track file');
      return;
    }
  ...
  const getSelectedAlbumName = () => {
    if (!formData.album_id) return 'Select Album';
    const album = albums.find((a: any) => String(a.id) === String(formData.album_id));
    return album?.title || 'Select Album';
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.pageTitle}>Create New Track</Text>
      ...
      <View style={styles.formContent}>
        ...
        <View style={styles.formGroup}>
          ...
          >
            <Text style={formData.album_id ? styles.selectorText : styles.selectorPlaceholder}>
              ...
              {albums.length > 0 ? (
                albums.map((album: any) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.dropdownItem,
                      String(album.id) === String(formData.album_id) && styles.dropdownItemActive,
                    ]}
                    onPress={() => {
                      setFormData((prev) => ({ ...prev, album_id: String(album.id) }));
                      setShowAlbumPicker(false);
                    }}
                  >
                    <Text style={styles.dropdownItemText}>{album.title}</Text>
                  </TouchableOpacity>
                ))
              ) : (
                <View style={styles.dropdownItem}>
                  <Text style={styles.dropdownItemTextEmpty}>No albums available</Text>
                </View>
              )}
            </View>
          )}
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>
            Duration (seconds) <Text style={styles.required}>*</Text>
          </Text>
          <TextInput
            style={styles.input}
            value={formData.duration_seconds > 0 ? String(formData.duration_seconds) : ''}
            onChangeText={(text) =>
              setFormData((prev) => ({ ...prev, duration_seconds: parseInt(text) || 0 }))
            }
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>
            Track File <Text style={styles.required}>*</Text>
          </Text>

          {!trackFile ? (
            <TouchableOpacity style={styles.uploadBox} onPress={handleTrackUpload}>
              <View style={styles.uploadIcon}>
                <Text style={styles.uploadIconText}>📤</Text>
              </View>
              <Text style={styles.uploadText}>Click to upload</Text>
              <Text style={styles.uploadHint}>or drag and drop</Text>
              <Text style={styles.uploadHint}>MP3, WAV, OGG up to 10MB</Text>
            </TouchableOpacity>
          ) : (
            <View style={styles.filePreview}>
              ...
              <View style={styles.fileInfo}>
                ...
                <Text style={styles.fileSize}>{formatFileSize(trackFile.size)}</Text>
              </View>
