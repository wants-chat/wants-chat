...
const TravelPlanDetail: React.FC<TravelPlanDetailProps> = ({ plan, onBack }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-bold">{plan.destination} Trip Details</h2>
      </div>
      ...
      <Card className="rounded overflow-hidden">
        ...
        <CardContent className="p-0">
          <div className="h-[400px] relative">
            ...
            {mapView === 'activities' && (
              <div className="absolute bottom-4 left-4 z-[1000] bg-white/10 backdrop-blur-xl border border-white/20 rounded shadow-lg p-3">
          ...
          <Card className="rounded lg:col-span-2">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded p-4">
                  <p className="text-sm text-white/80 mb-1">Total Budget</p>
                  <p className="text-2xl font-bold text-white">${expenseData.totalBudget}</p>
                </div>
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded p-4">
                  <p className="text-sm text-white/80 mb-1">Estimated Expenses</p>
                  <p className="text-2xl font-bold text-white">${expenseData.totalSpent.toFixed(0)}</p>
                </div>
                <div className={`rounded p-4 ${expenseData.savingsOrOverspend >= 0 ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className={`text-2xl font-bold ${expenseData.savingsOrOverspend >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-medium">{expenseData.percentageUsed.toFixed(1)}%</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, expenseData.percentageUsed)}%` }}
                  />
                </div>
                <div className="flex justify-between mt-1">
                  <span className="text-xs text-gray-500">$0</span>
                  <span className="text-xs text-gray-500">${expenseData.totalBudget}</span>
                </div>
              </div>

              {/* Category Breakdown */}
              <div>
                <h4 className="font-semibold mb-3">Expense Breakdown by Category</h4>
          ...
          <Card className="rounded">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-blue-500/20 rounded p-4">
                <p className="text-sm text-blue-300 mb-1">Daily Average Spend</p>
                    ...
                    {[...Array(5)].map((_, i) => (
                      <div
                        ...
                        className={`h-2 w-8 rounded-full ${
                          i < Math.floor((100 - Math.min(100, expenseData.percentageUsed)) / 20) + 1
                            ? expenseData.percentageUsed <= 80
                              ? 'bg-green-500'
                              : expenseData.percentageUsed <= 100
                              ? 'bg-yellow-500'
